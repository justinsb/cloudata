...
public class GitOverHttpServlet extends GitServlet {
    ...
    @Inject
    GitRepositoryStore gitRepositoryManager;
    ...
    class Resolver implements RepositoryResolver<HttpServletRequest> {
        ...
        public Repository open(HttpServletRequest req, String projectName) throws RepositoryNotFoundException,
            ...
            final GitUser user = (GitUser) req.getAttribute(GitUser.class.getName());

            GitRepository repo;
            try {
                repo = gitRepositoryManager.findRepo(user, projectName);
            } catch (IOException e) {
            ...
            if (repo == null) {
                if (autoCreate) {
                    ...
                    try {
                        repo = gitRepositoryManager.createRepo(user, projectName);
                    } catch (IOException e) {
                        throw new RepositoryNotFoundException(projectName, e);
                    }
                } else {
            ...
            try {
                ...
                return gitRepositoryManager.openRepository(user, repo, mustExist);
            } catch (IOException e) {
                throw new RepositoryNotFoundException(projectName, e);
            }
        }
